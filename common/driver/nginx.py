...
class NginxDriver(object):
    ...
    def _add_host(self,server,server_path):
        if not os.path.exists(server_path):
            try:
                ...
                self.logger.exception(e)
                self.delete_server_path(server_path)    # if nginx action perform faild, delete host config file.
    ...
    def add_host(self, message):
        ...
        try:
            if not host_type:
                raise AgentNoHostTypeError
        except AgentNoHostTypeError,e:
            self.logger.exception(e)
            return False,'hosttype_missed'
    ...
    def _delete_http_host(self, server):
        server_path = os.path.join(settings.http_conf_path, server + settings.conf_suffix)
        ret = self._delete_host(server_path)
        return ret
    
    def _delete_tcp_host(self, server):
        server_path = os.path.join(settings.tcp_conf_path, str(server) + settings.conf_suffix)
        ret = self._delete_host(server_path)
        return ret
    ...
    def perform_conf(self):
        ...
        self.logger.debug('reload nginx now...')
        action = ' -s reload'
        ...
        if returncode == 1:
            ...
            raise NginxPerformError(exc=output)
    
    def ensure_conf(self,conf_path):
        """
        make sure the configuration path and the executeable file of nginx are exists. 
        """
        try:
